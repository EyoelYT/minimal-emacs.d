...
(defun magit-project-status ()
  "Prompt to select a project using project.el, then run `magit-status' in
its root."
  (interactive)
  (let ((projects (project-known-project-roots)))
    (if projects
        (let* ((project-dir (completing-read "Switch to project: " projects nil t))
...
root."
  ...
  (let ((projects (project-known-project-roots)))
    (if projects
        (let* ((project (completing-read "Switch to project: " projects))
               (magit-display-buffer-function #'magit-display-buffer-same-window-except-diff-v1)
               (default-directory project))
          ;; magit-log-setup-buffer sets up the log view natively in the default-directory
          (require 'magit-log)
          (magit-log-setup-buffer nil '("--branches" "--remotes" "--graph" "--color" "-n256" "--decorate") nil))
      (user-error "There are no known projects"))))
#+end_src
