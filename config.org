...
(use-package evil
  ...
  (setq evil-want-C-i-jump nil)         ; don't bind TAB, bind [C-i] instead
...
(use-package org
  ...
  (setq org-edit-src-content-indentation 0
        ...
        org-clock-clocktable-default-properties (list :maxlevel 6 :narrow nil)
        org-duration-format (quote h:mm)
        org-startup-indented t
        org-hide-emphasis-markers nil
        ;; org-agenda-files '("~/temp_agenda.org")
  ...
  (defun auto-org-babel-tangle-async ()
    "Tangle the current Org file asynchronously in the background."
    (let ((file (buffer-file-name)))
      (message "Tangling %s in the background..." (file-name-nondirectory file))
      (async-start
       `(lambda ()
          (require 'org)
...
(custom-set-faces
 ...
 `(line-number-current-line ((t (:background ,(face-attribute 'line-number :background)))))
...
(use-package git-timemachine
  ...
  :defer t
  :config
  (defun +git-timemachine-update-header-line-a (revision)
  ...
  (advice-add 'git-timemachine--show-minibuffer-details :override #'+git-timemachine-update-header-line-a))
...
(use-package magit
  ...
  (when (magit-git-version>= "2.55.0")
      (transient-append-suffix 'magit-log "-o"
    '("-z" "Start from the first commit" "--max-count-oldest=")))
...
(use-package autorevert
  ...
  :ensure nil
  ;; :hook (focus-in . doom-auto-revert-buffers-h) ; FIXME: may need to use `after-focus-change-function'
...
(use-package vertico
  ...
  (setq vertico-multiform-commands
        `(("consult-\\(.*\\)?\\(find\\|grep\\|ripgrep\\)" ,@prot-vertico-multiform-maximal)
          ("ey/consult-\\(.*\\)?\\(find\\|grep\\|ripgrep\\)" ,@prot-vertico-multiform-maximal)))
...
(use-package vertico
  ...
  (setq vertico-flat-max-lines 4)
  (add-hook 'minibuffer-setup-hook #'vertico-repeat-save)
  (add-hook 'rfn-eshadow-update-overlay-hook #'vertico-directory-tidy)
  ;; (setq vertico-flat-format               ; remove space
  ;;       '(:multiple #("{%s}" 0 1 (face minibuffer-prompt) 3 4 (face minibuffer-prompt) 0 1 (cursor t))
  ;;         :single   #("[%s]" 0 1 (face minibuffer-prompt) 3 4 (face minibuffer-prompt) 0 1 (cursor t))
  ;;         :prompt   #("(%s)" 0 1 (face minibuffer-prompt) 3 4 (face minibuffer-prompt) 0 1 (cursor t))
  ;;         :separator #(" | " 0 3 (face minibuffer-prompt))
  ;;         :ellipsis  #("..." 0 3 (face minibuffer-prompt))
  ;;         :no-match  #("[No match]" 0 1 (cursor t))
  ;;         :spacer    #("")))
  (setq vertico-flat-format
        '(:multiple   #("{%s}" 0 1 (face minibuffer-prompt) 3 4 (face minibuffer-prompt))
...
;;   :config
;;   (setq kanagawa-themes-org-height nil)) ;; ERROR: inheritance cycle error
...
;; I like modus based themes, they cover a lot more faces
(use-package folio-theme :ensure (:host github :repo "kn66/folio-theme.el"))
(use-package kusanagi-theme :ensure (:host github :repo "LionyxML/kusanagi-theme"))
(use-package peppers-theme :ensure (:host codeberg :repo "joar/peppers-theme"))
(use-package modus-nordic-night :ensure (:host codeberg :repo "ashton314/modus-nordic-night"
                                               :main "modus-nordic-night-theme.el"))
#+end_src
...
* Markdown
#+begin_src emacs-lisp :tangle post-init.el
;; (use-package markdown-mode ; FIXME: replace with `markdown-ts-mode'
;;   :defer t
...
** Magit Window functions
#+begin_src emacs-lisp :tangle post-init.el
(defun magit-fetch-from-upstream-all-windows ()
  "Loop over all visible windows in the current frame,and if the buffer is
in `magit-mode`, fetch from upstream."
  (interactive)
  (let ((processed-dirs '()))
      ...
      (let ((buf (window-buffer win)))
        (with-current-buffer buf
          (if (and (derived-mode-p 'magit-mode)
                   (not (member default-directory processed-dirs)))
              (progn
                (push default-directory processed-dirs)
                (magit-with-toplevel
                  (message "Fetching for %s" (buffer-name buf))
                  (call-interactively #'magit-fetch-from-upstream)))
            (message "Skipping %s" (buffer-name buf))))))))

(defun magit-pull-from-upstream-all-windows ()
  "Loop over all visible windows in the current frame,and if the buffer is
in `magit-mode`, pull from upstream."
  (interactive)
  (let ((processed-dirs '()))
    (dolist (win (window-list))
      (let ((buf (window-buffer win)))
        (with-current-buffer buf
          (if (and (derived-mode-p 'magit-mode)
              ...
              (progn
                (push default-directory processed-dirs)
                (magit-with-toplevel
                  (message "Pulling for %s" (buffer-name buf))
...
all hooks after it are ignored.")
  ...
  (defun +evil-disable-ex-highlights-h ()
    ...
    (when (or (and (fboundp 'evil-ex-hl-active-p)
                   (evil-ex-hl-active-p 'evil-ex-search))
              (bound-and-true-p anzu--state))
      (evil-ex-nohighlight) t))

  (defun +evil-quit-multiedit-h ()
    "Quit multiedit mode and return t."
    (when (and (bound-and-true-p evil-multiedit-mode)
               (fboundp 'evil-multiedit-abort))
      (evil-multiedit-abort) t))

  (add-hook 'doom-escape-hook #'+evil-disable-ex-highlights-h)
  (add-hook 'doom-escape-hook #'+evil-quit-multiedit-h)
  ...
  ;; Run this before `evil-multiedit-mode's advice to `evil-force-normal-state' runs
  (advice-add 'evil-force-normal-state :before #'+evil-escape-a '((depth . -50)))
  ...
  (defun doom/escape (&optional interactive)
    ...
    (let ((inhibit-quit t))
      (cond (;; Run all escape hooks. If any returns non-nil, then stop there.
            ...
            ;; quit the minibuffer if minibuffer is active and point is in the minibuffer.
...
(defun ey/compile (command &optional comint)
  ...
  (compilation-start command (not comint)))
#+end_src
...
Also add the value to the front of the list in the variable `values'."
  (interactive
   (let ((selection
          (if (use-region-p)
            ...
            nil)))
     (list (read--expression "Eval: " selection) current-prefix-arg)))
  (message "Evaluating...")
  (let ((result (eval expression lexical-binding)))
    ...
    (if insert-value
      ...
      (pp-display-expression result "*Eval Output*" pp-use-max-width))))
#+end_src
...
#+begin_src emacs-lisp :tangle post-init.el
(defun ey/consult-search-project ()
...
conduct a text search from the current directory"
  ...
  (let ((selected-text (when (region-active-p)
                         (buffer-substring-no-properties (region-beginning) (region-end)))))
    (if (project-current)               ; REVIEW: we can maybe directly use consult-ripgrep here
        (consult-ripgrep (project-root (project-current)) selected-text)
      (ey/consult-ripgrep-cwd))))

(defun ey/consult-search-other-project ()
  "Conduct a text search from the root of a known project. If there are no
known projects, throw an error."
  (interactive)
  (if (not (project-known-project-roots))
      (user-error "There are no known projects")
    (let* ((selected-text (when (region-active-p)
                            (buffer-substring-no-properties (region-beginning) (region-end))))
           (project-dir (project-prompt-project-dir)))
      (when (and project-dir (not (string-empty-p project-dir)))
...
#+begin_src emacs-lisp :tangle post-init.el
;;; Visual line-extension commands

(defun ey--first-non-blank-in-current-line ()
  "Position of the first non-blank char of the current line.
Returns `line-beginning-position' on a blank line."
  (save-excursion
    (back-to-indentation)
    (if (eolp) (line-beginning-position) (point))))

(defun ey--last-non-blank-in-current-line ()
  "Position of the last non-blank char of the current line's code.
Ignores a trailing comment and trailing whitespace. Returns
`line-beginning-position' when the line has no such char."
  (save-excursion
    (doom/forward-to-last-non-comment-or-eol) ; point ends up *after* last code char
    (skip-chars-backward " \t")
    (max (line-beginning-position) (1- (point)))))

(defun ey--visual-extend-to (target-fn)
  "Enter (or keep) charwise Visual state, extending the selection to (TARGET-FN).
TARGET-FN is called with no arguments and must return a buffer position.
Any existing selection anchor is preserved; line-/block-wise selections
are normalized to charwise."
  ;; leave insert/replace/emacs state *without* moving point
  (unless (or (evil-visual-state-p) (evil-normal-state-p) (evil-motion-state-p))
    (let ((evil-move-cursor-back nil))
      (evil-change-to-initial-state)
      (unless (or (evil-normal-state-p) (evil-motion-state-p))
        (evil-normal-state 1))))
  ;; anchor: keep the existing one in visual state, else start at point
  (let ((anchor (if (evil-visual-state-p) (or (mark t) (point)) (point))))
    ;; move to the target
    (goto-char (funcall target-fn))
    ;; (re)build the selection
    (evil-visual-make-selection anchor (point) 'char)))

(defun ey/visual-to-first-non-blank-in-current-line ()
  "Visual state, extending the selection to the first non-blank char of the line."
  (interactive)
  (ey--visual-extend-to #'ey--first-non-blank-in-current-line))

(defun ey/visual-to-last-non-blank-in-current-line ()
  "Visual state, extending the selection to the last non-blank char of the line."
  (interactive)
  (ey--visual-extend-to #'ey--last-non-blank-in-current-line))

;; Without :keep-visual, evil expands the region before the command (off-by-one)
;; and exits Visual state after it (that's the state-dependent weirdness).
(dolist (cmd '(ey/visual-to-first-non-blank-in-current-line
               ey/visual-to-last-non-blank-in-current-line))
  (evil-add-command-properties cmd :keep-visual t :repeat 'abort))
...
(defun google-search (query)
  ...
  (cond
   ...
   (t
    (browse-url (concat "https://www.google.com/search?q=" (url-hexify-string query) "&udm=50&ie=UTF-8")))))
#+end_src
...
(use-package buffer-terminator
  ...
  (setopt buffer-terminator-verbose t
          buffer-terminator-inactivity-timeout (* 3 60 60) ; 3 hours
          buffer-terminator-interval (* 60 60))) ; 1 hour
#+end_src
...
(use-package treesit
  ...
  :config
  (add-hook 'python-ts-mode-hook '+treesit-custom-colors-mode)
  (add-hook 'c++-ts-mode-hook '+treesit-custom-colors-mode)
  (add-hook 'c-ts-mode-hook '+treesit-custom-colors-mode)
  (add-hook 'typescript-ts-mode-hook '+treesit-custom-colors-mode)
  (add-hook 'tsx-ts-mode-hook '+treesit-custom-colors-mode)
...
#+begin_src emacs-lisp :tangle post-init.el
(defvar-local +treesit-mute-cookies nil
  "List of cookies for muted face remappings.")

(define-minor-mode +treesit-custom-colors-mode
  "Control faces, intended for treesit modes. Best used alongside
`treesit-font-lock-level' set to 4"
  :local t
  :init-value nil
  :lighter " TS-Mute"
  (dolist (cookie +treesit-mute-cookies)
    (face-remap-remove-relative cookie))
  (setq +treesit-mute-cookies nil)
  (when +treesit-custom-colors-mode
    (setq +treesit-mute-cookies
          (list (face-remap-add-relative 'font-lock-variable-use-face 'default)
                (face-remap-add-relative 'font-lock-property-use-face 'default)
                (face-remap-add-relative 'font-lock-function-call-face 'default)))))
...
(add-hook 'typescript-ts-mode-hook '+treesit-custom-colors-mode)
(add-hook 'tsx-ts-mode-hook '+treesit-custom-colors-mode)
(add-hook 'odin-ts-mode-hook '+treesit-custom-colors-mode)
#+end_src
...
(keymap-set prog-mode-map "C-c C-p" 'beginning-of-defun)
(keymap-set prog-mode-map "C-c C-n" 'end-of-defun)
#+end_src
...
(use-package jinx
  ...
  (with-eval-after-load 'evil
    ...
    (evil-define-key 'motion global-map (kbd "]s") #'jinx-next)))
#+end_src
...
(use-package evil-ghostel
  ...
  (evil-define-key 'motion ghostel-semi-char-mode-map (kbd "C-s") 'ey/window-search)
...
(with-eval-after-load 'evil
  ...
  (evil-define-key '(normal visual) global-map (kbd "C-M-d") 'evil-multiedit-restore)
...
(with-eval-after-load 'evil-collection-org
...
(with-eval-after-load 'evil-collection-magit
  (evil-define-key* '(normal) magit-section-mode-map
    ...
    (kbd "M-3")         nil
    (kbd "M-4")         nil
    (kbd "C-S-<tab>")  'magit-section-cycle-global
    (kbd "S-<tab>")    'magit-section-cycle
    (kbd "C-<tab>")    'evil-switch-to-windows-last-buffer
    (kbd "<backtab>")  'magit-section-cycle
    (kbd "C-S-<down>") 'magit-section-forward
    (kbd "C-S-<up>")   'magit-section-backward
    (kbd "M-RET")      'magit-diff-visit-worktree-file-other-window
    (kbd "S-<return>") 'magit-diff-visit-worktree-file-other-window)
...
** Minibuffer Mappings
#+begin_src emacs-lisp :tangle post-init.el
(defvar +default-minibuffer-maps '(minibuffer-local-map
...
(define-leader-keys
  ...
  "h I" '(consult-info                    :which-key "consult-info")
  "\""  '(consult-complex-command         :which-key "consult-complex-command")
  "s e" '(consult-compile-error           :which-key "consult-compile-error")
  "s F" '(consult-flymake                 :which-key "consult-flymake")
  ...
  "s I" '(consult-imenu-multi             :which-key "consult-imenu-multi")
  "s S" '(consult-line-multi              :which-key "consult-line-multi")
...
(with-eval-after-load 'evil
  (evil-define-key '(insert emacs) eshell-mode-map (kbd "C-s") 'consult-history))
#+end_src
...
(define-leader-keys
 ...
 "s M" '(global-mode-line-invisible-mode           :which-key "toggle modeline visibility")
...
(define-leader-keys
 ...
 "b I"   '(ibuffer-other-window               :which-key "ibuffer other window")
 ...
 "<"     '(consult-buffer-other-window        :which-key "consult buffer in other window")
 "b o"   '(consult-buffer-other-window        :which-key "consult buffer in other window")
...
(define-leader-keys
 ...
 "t L" '(global-display-line-numbers-mode          :which-key "toggle global line numbers")
...
(define-leader-keys
 ...
 "g B" '(magit-blame-addition           :which-key "magit-blame-addition")
 "g /" '(magit-dispatch                 :which-key "magit-dispatch")
 "g ." '(magit-file-dispatch            :which-key "magit-file-dispatch")
 "f g" '(magit-file-dispatch            :which-key "magit-file-dispatch")
 ...
 "g x" '(diff-hl-revert-hunk            :which-key "diff-hl-revert-hunk")
...
#+begin_src emacs-lisp :tangle post-init.el
(use-package purr
  :ensure (:host sourcehut :repo "zelda/purr.el")
  :defer t)
#+end_src

* Current
#+begin_src emacs-lisp :tangle post-init.el
(defvar current-act nil
  "The house of the current act.")

;; (defvar current-act-modeline-format
;;   '(:eval (propertize (format " [CA: %s]" current-act)
;;                       'face 'bold
;;                       'help-echo "The Current Act")))

(defun current-act-set (act)
  "Set the `current-act'."
  (interactive (let ((selected-text (if (use-region-p)
                                        (buffer-substring-no-properties
                                         (region-beginning) (region-end)))))
                 (list (read-string "set current act: " selected-text))))
  (setq current-act act)
  (force-mode-line-update t)
  (message "Current act updated to: %s" act))

(defun current-act-unset ()
  "Unset the `current-act'"
  (interactive)
  (setq current-act nil))

(defun current-act-show-in-minibuffer ()
  "Display the `current-act' in the minibuffer."
  (interactive)
  (message "Current Act: %s" current-act))

(define-leader-keys
  "A s" 'current-act-set
  "A m" 'current-act-show-in-minibuffer
  "A u" 'current-act-unset)
#+end_src

* Modeline Invisible Mode                                               :ppr:
#+begin_src emacs-lisp :tangle post-init.el
(defcustom global-mode-line-invisible-mode-excluded-modes '()
  "Major modes unaffected by `global-mode-line-invisible-mode'.

Modes derived from an element of this list are also excluded."
  :type '(repeat symbol)
  :group 'mode-line)

(defun global-mode-line-invisible-mode--excluded-p ()
  (if (fboundp 'derived-mode-all-parents)
      (derived-mode-p global-mode-line-invisible-mode-excluded-modes)
    (apply #'derived-mode-p global-mode-line-invisible-mode-excluded-modes)))

(defun global-mode-line-invisible-mode--turn-on ()
  "Enable `mode-line-invisible-mode' in the current buffer."
  (unless (or (bound-and-true-p mode-line-invisible-mode)
              (minibufferp)                       ; skip minibuffer
              (string-prefix-p " " (buffer-name)) ; skip internal buffers
              (global-mode-line-invisible-mode--excluded-p))
    (mode-line-invisible-mode 1)))

(define-globalized-minor-mode global-mode-line-invisible-mode
  mode-line-invisible-mode
  global-mode-line-invisible-mode--turn-on
  :group 'mode-line)
#+end_src

* Proj Packages
org clock to csv for example
#+begin_src emacs-lisp :tangle post-init.el
#+end_src

* File Local Variables
# Local Variables:
# eval: (add-hook 'after-save-hook #'auto-org-babel-tangle-async nil t)
# End:
